...
public:
    ...
    virtual void operator()(const ValueType& val) { m_values.push_back(val); };
    virtual void getResult(ValueType& val) { getMean(m_values, val); };
    virtual void getResultAndSigma(ValueType& val, typename vigra::NumericTraits<ValueType>::RealPromote& sigma) { getMeanSigma(m_values, val, sigma); };
    virtual bool IsValid() { return !m_values.empty(); };
    const std::string getName() const { return "mean"; };
protected:
    std::vector<ValueType> m_values;
};

template<class ValueType>
class MedianStacker : public AverageStacker<ValueType>
{
public:
    virtual void getResult(ValueType& val) override
    {
        sort();
        if (this->m_values.size() % 2 == 1)
        {
            val = this->m_values[(this->m_values.size() - 1) / 2];
        }
        else
        {
            const int index = this->m_values.size() / 2;
            val = 0.5 * (this->m_values[index - 1] + this->m_values[index]);
        };
    };
    virtual void getResultAndSigma(ValueType& val, typename vigra::NumericTraits<ValueType>::RealPromote& sigma) override
    {
        getResult(val);
        ValueType mean;
        getMeanSigma(this->m_values, mean, sigma);
    };
...
protected:
    ...
    {
        std::sort(this->m_values.begin(), this->m_values.end());
    };
    // sort color values
...
private:
    void SigmaClip()
    {
        ...
        while (iteration < Parameters.maxIterations)
        {
            ...
            {
                // no values outside range, end while loop
                break;
            };
            ++iteration;
        };
...
public:
    ...
    {
        if (mask > 0 && (color.luminance() < limits[0].luminance() || color.luminance() > limits[1].luminance()))
        {
            return 255;
        }
        else
...
bool StackImagesAndMask(std::vector<InputImage*>& images, Functor& stacker)
{
    ...
    vigra::BasicImage<PixelType> output(outputROI.size());
    vigra::BImage mask(output.size(), vigra::UInt8(0));
    vigra::BasicImage<vigra::TinyVector<PixelType, 2>> limits(output.size());
    // loop over all lines
    for (size_t y = outputROI.top(); y < outputROI.bottom(); ++y)
    {
        // load next line
...
bool main_stacker(std::vector<InputImage*>& images)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                std::cout << "Merging stack with mean operator." << std::endl;
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (Parameters.stackMode.empty())
                            {
                                std::cerr << "ERROR: No stacking mode given. Please specify a stacking mode with --mode=STACK_MODE" << std::endl
...
int main(int argc, char* argv[])
{
    ...
    while (i < nFiles)
    {
        // @ indicates response file follows
        if (argv[optind + i][0] == '@')
        {
            ++i;
            isResponseFile = true;
        ...
        {
            // read response file
            std::ifstream textfile(currentFile);
            if (textfile.is_open())
            {
                std::string line;
                ...
                while (std::getline(textfile, line))
                {
                    ...
                    if (line.empty() || (pos != std::string::npos && line[pos] == '#'))
                    {
                        // ignore empty lines and lines beginning if "#"
                        // these are regarded as comments
                        continue;
                    };
                    // check if file exists and it is an image file
                    if (hugin_utils::FileExists(line) && vigra::isImage(line.c_str()))
                    {
        ...
        {
            // image file given, check file existence
            if (hugin_utils::FileExists(currentFile))
            {
                if (vigra::isImage(currentFile.c_str()))
                {
                    // it is an image file
                ...
                {
                    std::cerr << "ERROR: File " << currentFile << " is not a image file recognized by vigra." << std::endl;
                };
            }
            else
            {
                std::cerr << "ERROR: File " << currentFile << " does not exists." << std::endl;
            };
        };
        ++i;
    };
    ...
    for (size_t i = 0; i < files.size(); ++i)
    {
        ...
        if (images.back()->getROI().area() == 0)
        {
            std::cerr << "ERROR: Image " << images.back()->getFilename() << " has no valid data." << std::endl;
            CleanUp(images);
            return 1;
        };
    };
