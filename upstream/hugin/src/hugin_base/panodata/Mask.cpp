...
void MaskPolygon::scale(const double factor, const hugin_utils::FDiff2D& p)
{
    for (unsigned int i = 0; i < m_polygon.size(); i++)
    {
        m_polygon[i] = (m_polygon[i] - p) * factor + p;
    };
    calcBoundingBox();
}
...
void MaskPolygon::calcBoundingBox()
{
    if(!m_polygon.empty())
    {
        m_boundingBox = vigra::Rect2D(vigra::Point2D(m_polygon[0].x, m_polygon[0].y), vigra::Size2D(1, 1));
        if(m_polygon.size()>1)
        {
...
hugin_utils::FDiff2D MaskPolygon::getCenter() const
{
    if (m_polygon.empty())
    {
        return hugin_utils::FDiff2D();
    }
    else
    {
        ...
        return center / (double)m_polygon.size();
    };
}
