                         ...
                         const wxString& name)
{
    ...
    Bind(wxEVT_TIMER, &CPImageCtrl::OnSizeFinished, this, m_resizeTimer.GetId());
    ...
    Bind(wxEVT_TIMER, &CPImageCtrl::OnTimer, this, m_timer.GetId());
...
void CPImageCtrl::rescaleImage()
{
    ...
    if (!m_img->iccProfile->empty() || huginApp::Get()->HasMonitorProfile())
    {
        ...
        // ignore icc profile for float images, because we have already color remapped image
        if (m_img->imageFloat->width() == 0)
        {
            iccProfile = *(m_img->iccProfile);
        }
        HuginBase::Color::CorrectImage(img, iccProfile, huginApp::Get()->GetMonitorProfile());
    };
    bitmap = wxBitmap(img);
...
void CPImageCtrl::OnSizeFinished(wxTimerEvent & e)
{
    ...
    if (!imageFilename.empty() && fitToWindow)
    {
        setScale(0);
    }
    e.Skip();
