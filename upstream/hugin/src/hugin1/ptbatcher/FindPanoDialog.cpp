...
// provide some more translations for strings in wxDirPickerCtrl
#if 0
_("Browse");
#endif
...
FindPanoDialog::FindPanoDialog(BatchFrame* batchframe, wxString xrcPrefix)
{
    ...
    m_dirPicker=XRCCTRL(*this,"find_pano_dir",wxDirPickerCtrl);
    ...
    hugin_utils::RestoreFramePosition(this, "FindPanoDialog");
...
{
    wxConfigBase* config=wxConfigBase::Get();
    hugin_utils::StoreFramePosition(this, "FindPanoDialog");
    config->Write("/FindPanoDialog/splitterPos", XRCCTRL(*this, "find_pano_splitter", wxSplitterWindow)->GetSashPosition());
    config->Write("/FindPanoDialog/actualPath",m_dirPicker->GetPath());
...
void FindPanoDialog::OnButtonStart(wxCommandEvent& e)
{
    ...
    {
        //start detection
        m_start_dir=m_dirPicker->GetPath();
